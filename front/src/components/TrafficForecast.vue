<template>
  <div class="main grid-container fluid">
    <breadcrumbs v-if="project" v-bind="{'header': 'Прогноз трафика из ПС', 'crumbs': [{'text': 'Мои проекты', 'link': ''}, {'text': project.domain, 'link': '#/project/' + project.projectId}]}"></breadcrumbs>
        ...
        <div class="large-6 cell" v-if="data">
          <table>
            <thead>
              <tr>
                <th>Запрос</th>
                <th class="text-right">Cпрос</th>
                <th class="text-right">Целевая позиция</th>
                <th class="text-right">Прогноз трафика из ПС</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="item in data" v-bind:key="item">
                <td>{{item.query}}</td>
                <td class="text-right">{{item.demand}}</td>
                <td class="text-right">{{item.targetPosition}}</td>
                <td class="text-right">{{item.traffiForecast}}</td>
              </tr>
            </tbody>
          </table>
