<template>
  <div class="main grid-container fluid">
    <breadcrumbs v-if="project" v-bind="{'header': 'Генерация текстов объявлений', 'crumbs': [{'text': 'Мои проекты', 'link': ''}, {'text': project.domain, 'link': '#/project/' + project.projectId}]}"></breadcrumbs>
    <div class="project" v-if="isLoading || isLoaded">
      ...
      <div v-if="isLoaded && !isLoading" class="grid-x">
        ...
        <div class="large-12 cell">
          ...
          <button class="button primary" v-on:click="submitFile()">Загрузить .xlsx с запросами и целевыми страницами и запустить генерацию</button>
        </div>
...
export default {
  ...
  methods: {
    ...
    handleFileUpload: function () {
      ...
      formData.set('domain', this.project.domain)
      HTTP.post(
        `GetAdTexts/`,
        formData,
        {
