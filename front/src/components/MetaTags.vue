<template>
  <div class="main grid-container fluid">
    <breadcrumbs v-if="project" v-bind="{'header': 'Генерация мета-тегов', 'crumbs': [{'text': 'Мои проекты', 'link': ''}, {'text': project.domain, 'link': '#/project/' + project.projectId}]}"></breadcrumbs>
    <div class="project" v-if="isLoading || isLoaded">
      ...
      <div v-if="isLoaded && !isLoading" class="grid-x">
        ...
        <div class="large-12 cell">
          ...
          <button v-bind:disabled="isLoading" class="button primary" v-on:click="submitFile()">Загрузить .xlsx с запросами и запустить генерацию</button>
        </div>
